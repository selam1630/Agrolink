...
import { GlobeIcon, UserIcon, MenuIcon, ShoppingCart } from "lucide-react"; 
import {
  Sheet,
  SheetContent,
  SheetTrigger,
...
const Header: React.FC<HeaderProps> = ({ onMenuClick }) => {
  ...
  // Correctly destructure cartItems from the useCart hook
  const { cartItems } = useCart();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  // Calculate the total number of items based on the cartItems array
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/cart" className="relative">
            ...
            >
              ...
              {/* Conditional rendering for the count badge */}
              {cartItemCount > 0 && (
          ...
          {/* This is the key change. We conditionally render the mobile menu button. */}
          {/* If onMenuClick is provided (e.g., from DashboardLayout), we use that to control the sidebar. */}
          {onMenuClick ? (
            <Button
              variant="ghost"
              size="icon"
