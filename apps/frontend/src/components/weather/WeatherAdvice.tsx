...
const WeatherDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    } else { setError("Geolocation not supported."); setIsLoading(false); }
  }, []);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const res = await fetch("http://localhost:5000/api/weather-prediction/advice", {
          method: "POST",
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-50 via-green-100 to-blue-50 flex flex-col items-center p-6 font-sans">
       <div className="w-full max-w-6xl mb-1 flex justify-end">
        <select
          value={language}
      ...
      <div className="w-full max-w-6xl grid md:grid-cols-3 gap-6">
       ...
       <p className="capitalize text-gray-700 mt-1 text-xs text-center truncate">{weatherData.weather[0].description}</p>
        ...
        {/* Advice Cards */}
        {[
          { title: "Weather & Forecast", content: weatherPrediction },
          { title: "Soil & Water Management", content: soilAndWaterAdvice },
          { title: "Pest & Disease Prevention", content: pestAndDiseaseAdvice },
          { title: "Recommended Crops", content: recommendedCrops },
          { title: "Emergency Preparedness", content: emergencyPreparedness },
          { title: "Location-Specific Tips", content: locationSpecificTips }
        ].map((section, idx) => (
          <div
            key={idx}
